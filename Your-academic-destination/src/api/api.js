...
// بدونها، لو الالخادم أو الـ tunnel وقع، الطلب بيضل معلّق للأبد والواجهة
...
export async function apiRequest(path, options = {}) {
  ...
  try {
    ...
    if (networkErr.name === 'AbortError') {
      throw new ApiError(
        'timeout',
        'استغرق الاتصال بالالخادم وقتاً أطول من المتوقع، تأكد من الإنترنت وحاول مرة اخرى',
        null
      );
    }

    // فشل الاتصال نفسه (الالخادم واقف، مافي إنترنت...) — قبل ما نوصل حتى لرد الباك
    throw new ApiError(
      'network_error',
