    ...
    const load = async () => {
      ...
      try {
        ...
        const response = await apiGet(
          `/admin/dashboard/registered-no-shows?${params.toString()}`
        );
        if (cancelled) return;
      ...
      <div className={`card-wrapper ${isSuperAdmin ? 'admin-gate-mode' : ''}`}>
        <div className="card-container">
          ...
          <main className="page-body">
            <h2 className="section-title">مسجّلون بلا حضور</h2>

            <div className="stats-row">
              <div className="stat-card">
                <span className="stat-label">إجمالي من سجّلوا ولم يحضروا</span>
                      ...
                      <tbody>
                        {items.map((item) => (
                          <tr key={item.unique_code}>
                            ...
                            <td className="time-cell">{formatDateTime(item.created_at)}</td>
                          </tr>
                        ))}
                        {items.length === 0 && (
                          <tr>
                            <td colSpan={4} className="empty-cell">
                              لا يوجد مسجّلون بلا حضور مطابقون للفلترة
                            </td>
                          </tr>
                        )}
                      </tbody>
...
export default RegisteredNoShowsPage;